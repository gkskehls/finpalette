...
2.  **계정 연동 및 데이터 동기화 (Account Sync):**
    ...
    - **시나리오 예시 (데이터 충돌 방지):**
      ...
      - **2단계 (PC):** 이후 사용자가 `PC`에서 동일한 계정으로 로그인합니다.
        - `PC`의 로\_컬 데이터(B)를 스캔하여 서버 ID가 없는 항목들을 서버에 업로드합니다. (데이터 B는 데이터 A와 ID가 다르므로 신규로 추가됨)
...
- **세부 기능:**
  1.  **프로필 정보 및 로그아웃:**
      - **로그인 상태:** 사용자 프로필 이미지, 닉네임, 이메일 및 로그아웃 버튼 표시.
      - **게스트 상태:** 로그인/회원가입을 유도하는 버튼 표시.
  2.  **게스트 저장 공간 관리 (게스트 전용):**
      - **표시 조건:** **로그아웃 상태(게스트 모드)에서만 표시**됩니다. 로그인 시 해당 섹션은 숨겨집니다.
      - **기능 목표:** 게스트 사용자에게 로컬 저장 공간의 한계를 시각적으로 인지시키고, 자연스럽게 계정 연동(회원가입)을 유도합니다.
      - **UI 구성 및 동적 변화:**
        - **기본 (70% 미만):** "현재 기기에 데이터를 저장하고 있어요." 문구와 함께 파란색 막대그래프로 현재 사용량 표시. (예: `[■■■■■■■□□□] 68%`)
        - **경고 (70% ~ 90%):** "저장 공간이 거의 다 차가고 있어요." 문구와 함께 주황색 막대그래프 및 경고 아이콘(⚠️)으로 주의 환기.
        - **위험 (90% 이상):** "저장 공간이 가득 찼습니다!" 라는 강력한 경고 문구와 붉은색 막대그래프 표시. 데이터 유실 방지를 위해 계정 연동을 촉구하는 **CTA(Call to Action) 버튼**을 명시적으로 노출. (예: `[ > 무료로 계정 연동하고 데이터 백업하기 < ]`)
  3.  **카테고리 관리 메뉴:** 클릭 시 `카테고리 관리 페이지`로 이동. 이 페이지에서 카테고리 추가/수정/삭제 및 색상 지정 가능.
  4.  **기타 설정 메뉴:** 앱 버전, 데이터 내보내기 등.
