...
- **상세 구현 계획 (단계별):**
  1.  **[DB] `categories` 테이블에 `sort_order` 컬럼 추가:**
      - 카테고리 순서를 저장하기 위한 `integer` 타입의 `sort_order` 컬럼을 추가한다.
      - 새 카테고리 생성 시, `sort_order` 값을 적절하게 설정하는 로직을 추가한다. (예: 가장 마지막 순서로 추가)

  2.  **[DB] `update_category_order` 함수 신설:**
      - **역할:** 카테고리 순서 변경을 처리하는 RPC 함수를 생성한다.
      - **인자:** `p_palette_id UUID`, `p_category_codes TEXT[]` (정렬된 카테고리 코드 배열)
      - **로직:** 배열을 순회하며 각 카테고리의 `sort_order` 값을 순서대로(1, 2, 3...) 업데이트한다.
      - **권한:** `owner` 또는 `admin`만 호출 가능하도록 설정한다.

  3.  **[Client] `useCategoryOrderMutation` 훅 신설:**
      - `update_category_order` RPC를 호출하는 React Query Mutation 훅을 생성한다.
      - **핵심 원칙 준수:** 이 훅 내부에서 `user` 객체의 존재 여부를 확인하여, 로그인 상태이면 RPC를 호출하고, **게스트 모드이면 `localStorage`의 카테고리 배열 순서를 직접 변경하고 `sort_order` 값을 업데이트**하는 로직을 구현한다.

  4.  **[Client] `CategorySettingsPage.tsx` 수정:**
      - `react-beautiful-dnd` 또는 `@dnd-kit/core` 라이브러리를 사용하여 드래그 앤 드롭 UI를 구현한다.
      - `onDragEnd` 콜백에서 변경된 순서로 상태를 업데이트하고, `useCategoryOrderMutation` 훅을 호출하여 변경 사항을 저장한다.

  5.  **[Client] 데이터 마이그레이션 로직 강화:**
      - 게스트가 로그인할 때, `localStorage`의 카테고리 목록을 `sort_order` 순으로 정렬하여 서버에 전달하고, 서버는 그 순서대로 `INSERT` 하도록 마이그레이션 스크립트를 수정한다.
