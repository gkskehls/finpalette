...
- **세부 작업:**
  1. **[진행중] 공유 가계부(팔레트) 기능**:
     ...
     - **[O] 시크릿 메모 기능 구현 (입력/조회)**
       - **[x] 알려진 이슈:** `private_memo`는 내역을 최초 등록한 `user_id`를 기준으로만 조회가 가능. 다른 사용자가 `private_memo`를 추가/수정하더라도 원본 등록자 외에는 볼 수 없는 문제가 있음. (추후 `private_memo`를 별도 테이블로 분리하여 작성자 기준으로 권한을 관리하는 방식으로 개선 필요)
     - **[O] 팔레트 생성 기능**
