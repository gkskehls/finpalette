...
- **세부 작업:**
  1.  **[ ] 거래 내역 리스트 UI 전면 개편 (Compact Design)**
      - 한 화면에 더 많은 정보가 보이도록 여백 및 폰트 크기 최적화
      - 아이콘 크기 축소 및 레이아웃 재배치
  2.  **[ ] 무한 스크롤 (Infinite Scroll) 구현**
      - 대량의 데이터 로딩 시 성능 저하 방지
      - 스크롤 하단 도달 시 추가 데이터 로딩 (Pagination)
  3.  **[ ] 텅 빈 화면 (Empty State) 디자인 적용**
      - 데이터가 없을 때(내역 없음, 멤버 없음 등) 사용자에게 안내 문구와 일러스트 제공
