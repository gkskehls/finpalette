...
- **세부 작업:**
  1. **✅ 로컬 스토리지 기반 데이터 관리 기능 구현 (Guest Mode)**:
     - TanStack Query를 도입하고, 로컬 스토리지를 사용하는 데이터 CRUD 훅(`useTransactionsQuery`, `useAddTransactionMutation`)을 구현했습니다.
     - 로그인하지 않은 사용자의 데이터가 브라우저(Local Storage)에 저장되고 관리됩니다.
  2. **✅ 기본 레이아웃 구현**: 앱 전체의 헤더, 하단 탭 네비게이션, 플로팅 액션 버튼 등 기본 구조를 구현했습니다.
  3. **✅ 대시보드 페이지 구현 (v2: 로컬 스토리지 연동)**:
     - 기존 목업 데이터를 제거하고, 로컬 스토리지와 연동된 데이터 훅을 사용하여 실제 데이터가 표시되도록 리팩토링했습니다.
     - 월별 요약, 카테고리별 지출, 최근 거래 내역이 동적으로 표시됩니다.
  4. **✅ 빠른 내역 추가 기능**:
     - 플로팅 액션 버튼(+)과 내역 입력 폼(`TransactionFormModal`)을 구현했습니다.
     - 폼 입력을 통해 로컬 스토리지에 데이터를 추가하고, 대시보드에 즉시 반영되는 기능을 완성했습니다.
