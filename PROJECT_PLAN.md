...
- **세부 작업:**
  ...
  2. **[O] UI/UX 개선**
     ...
     - **[O] 내역 입력 폼 UX 개선 (날짜 너비 조정, 카테고리 자동 선택)**
       - [O] 카테고리 선택 UI 개선 (아코디언 방식의 아이콘 그리드 적용)
       - [O] 수정 모드에서 긴 메모 작성을 위해 `<textarea>` 적용 및 자동 높이 조절
       - [O] 금액 입력 시 쉼표(,) 자동 포맷팅 기능 추가
       - [O] 내역 입력 중 카테고리 관리 페이지 이동 기능 추가 (데이터 유실 방지 컨펌 모달 적용)
     - **[O] 프로필 관리 개선 (이미지 업로드, 리사이징, 이름 수정)**
       - [O] 프로필 수정 시 Auth 세션 메타데이터 동기화 (FOUC 방지)
       - [O] 프로필 페이지 진입 시 DB 데이터와 세션 데이터 자동 동기화 (Self-Healing)
     - **[O] 내역 수정 모달 UI 버그 수정**
       - [O] 모달 z-index 조정으로 FAB 오버레이 문제 해결
       - [O] 카테고리 선택기 너비 문제 해결 (Flexbox overflow)
       - [O] 수정 모드에서 수입/지출 탭 전환 시 카테고리 목록이 갱신되지 않는 버그 수정
       - [O] 카테고리 선택기 너비를 콘텐츠에 맞게 동적으로 조절 (fit-content)
       - [O] 카테고리 선택기 아코디언 UI 동작 방식 수정 (팝업 -> 인라인)
       - [O] 수정 모드 진입 시 기존 카테고리가 초기화되는 버그 수정
     - **[O] 권한 관리 시스템 강화**
       - [O] 역할 기반 UI 제어 (버튼 숨김/비활성화)
       - [O] Mutation에 `onError` 핸들러를 추가하여 권한 오류 발생 시 명확한 피드백 제공
       - [O] 상세 보기와 수정/삭제 분리: 권한 없는 사용자에게는 '읽기 전용' 모달 제공
       - [O] 게스트 모드 권한 오류 수정
     - **[O] 기본 카테고리 정의 일원화**
       - [O] `constants.ts` 기준으로 `supabase_schema.sql` 및 관련 문서의 카테고리 정의 통일
  3. **[O] 내용/카테고리 기반 거래 내역 검색 기능**
  4. **[O] 카테고리 순서 변경 기능 (드래그 앤 드롭)**
  5. **[X] 반복 내역(고정 지출/수입) 자동 등록 기능**
  6. **[O] PWA(Progressive Web App) 적용 및 아이콘 설정**
  7. **[X] 데스크탑 화면 최적화**
...
- **세부 작업:**
  ...
  5.  **[O] 로딩 스켈레톤 (Skeleton UI) 적용**
      - [O] 데이터 로딩 중 깜빡임 방지 및 체감 속도 향상
  6.  **[O] 페이지 이동 시 스크롤 최상단 이동 처리**
      - [O] `App.tsx`에서 `useLocation`을 활용하여 경로 변경 시 `window.scrollTo(0, 0)` 실행
  7.  **[O] 달력 뷰 (Calendar View) 도입**
      - [O] 전체 내역 페이지에 리스트/달력 토글 기능 추가
      - [O] 월별 수입/지출 합계를 보여주는 달력 그리드 UI 구현
      - [O] 날짜 선택 시 하단에 해당 일자 상세 내역 표시 기능 구현
      - [O] 달력에서 날짜 선택 후 내역 추가 시 해당 날짜 자동 입력 기능 구현
      - [O] 달력 월 이동 시 해당 월 데이터 자동 로딩 기능 구현 (리스트 뷰와 데이터 분리)
  8.  **[O] 통계 페이지 데이터 로딩 개선**
      - [O] 통계 페이지에서 월 이동 시 해당 월의 데이터를 독립적으로 로딩하도록 수정 (`useCalendarTransactionsQuery` 재사용)
  9.  **[O] 모바일 터치 피드백 및 애니메이션 강화**
      - [O] 버튼 및 리스트 아이템 터치 시 `:active` 효과 적용
      - [O] 페이지 전환 시 `fade-in/out` 애니메이션 적용
  10. **[O] 동적 테마 시스템 고도화**
      - [O] HSL 기반 동적 색상 적용
      - [O] 인지 휘도(Perceived Luminance) 기반 텍스트 색상 자동 결정 (가독성 개선)
      - [O] 다크 모드 전용 채도/밝기 조정 로직 추가 (시각적 편안함 증대)
      - [O] 초기 로딩 시 테마 색상 깜빡임(Flickering) 현상 해결 (스켈레톤 테마 적용)

### Phase 5: 확장 - 알림 및 고급 기능 (Future)
...
## 7. 추후 논의 및 개선 사항 (Backlog)

- **[O] 색상 선택 UI 개선:** 팔레트 생성/수정 시 제공되는 원형 컬러 그리드가 모바일 화면에서 너무 많은 공간을 차지함. 더 컴팩트한 UI로 개선하고, 프리셋 외에 사용자가 직접 색상 코드(Hex)를 입력하거나 선택할 수 있는 '커스텀 컬러 피커' 기능 도입 필요.
- **[O] 카테고리 생성 로직 개선 및 사용자 경험 일원화:** 상세 내용은 `APP_SPECIFICATION.md`의 `5.1` 항목 참조.
- **[O] 새 팔레트 생성 시 카테고리 복사 기능:** 상세 내용은 `APP_SPECIFICATION.md`의 `5.2` 항목 참조.
- **[O] 게스트 모드 완전 동기화:** 상세 내용은 `APP_SPECIFICATION.md`의 `5.4` 항목 참조.
